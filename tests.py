...
class RedisProtocolTest(TestCase):
    ...
    def test_lua_script(self, transport, protocol):
        ...
        return value * ARGV[1]
        '''
        yield From(protocol.set(u'foo', u'2'))
        ...
        # Call script.
        result = yield From(script.run(keys=[u'foo'], args=[u'5']))
        ...
        result = yield From(protocol.evalsha(
            script.sha, keys=[u'foo'], args=[u'5']))
        self.assertIsInstance(result, EvalScriptReply)
        result = yield From(result.return_value())
        self.assertEqual(result, 10)
        ...
        result = yield From(protocol.script_exists(
            [script.sha, script.sha, u'unknown-script']))
        self.assertEqual(result, [True, True, False])

        # Test script flush
        result = yield From(protocol.script_flush())
        self.assertEqual(result, StatusReply(u'OK'))
    ...
    def test_discard_transaction(self, transport, protocol):
        ...
        transaction = yield From(protocol.multi())
        yield From(transaction.set(u'my_key', u'b'))
    ...
    def test_password(self, transport, protocol):
        # Set password
        result = yield From(protocol.config_set(u'requirepass', u'newpassword'))
        ...
        with self.assertRaises(ErrorReply) as e:
            yield From(protocol.set(u'my-key', u'value'))
        self.assertEqual(
            e.exception.args[0], 'NOAUTH Authentication required.')

        # Reconnect:
        result = yield From(protocol.auth(u'newpassword'))
        self.assertIsInstance(result, StatusReply)

        # Redis queries should work again.
        result = yield From(protocol.set(u'my-key', u'value'))
        self.assertIsInstance(result, StatusReply)

        # Try connecting through new Protocol instance.
        transport2, protocol2 = yield From(connect(
            self.loop,
            lambda **kw: RedisProtocol(password=u'newpassword', **kw)))
        result = yield From(protocol2.set(u'my-key', u'value'))
        self.assertIsInstance(result, StatusReply)
        transport2.close()

        # Reset password
        result = yield From(protocol.config_set(u'requirepass', u''))
        self.assertIsInstance(result, StatusReply)
    ...
    def test_set_scan(self, transport, protocol):
        ...
        Test sscan '''
        size = 1000
        items = [u'value-%i' % i for i in range(size)]

        # Create a huge set
        yield From(protocol.delete([u'my-set']))
        yield From(protocol.sadd(u'my-set', items))

        # Scan this set.
        cursor = yield From(protocol.sscan(u'my-set'))
    ...
    def test_alternate_gets(self, transport, protocol):
        ...
        # Test mget_aslist
        result = yield From(protocol.mget_aslist([u'my_key', u'my_key2']))
        self.assertEqual(result, [u'a', u'b'])
        self.assertIsInstance(result, list)

        # Test keys_aslist
        result = yield From(protocol.keys_aslist(u'some-prefix-'))
        self.assertIsInstance(result, list)
        ...
        # Test hgetall_asdict
        result = yield From(protocol.hgetall_asdict(u'my_hash'))
        self.assertEqual(result, {u'a': u'1', u'b': u'2', u'c': u'3'})
        self.assertIsInstance(result, dict)

        # test all inside a transaction.
        transaction = yield From(protocol.multi())
        f1 = yield From(transaction.mget_aslist([u'my_key', u'my_key2']))
        f2 = yield From(transaction.smembers_asset(u'my_set'))
        f3 = yield From(transaction.hgetall_asdict(u'my_hash'))
    ...
    def test_cancellation(self, transport, protocol):
        ...
        '''
        yield From(protocol.delete([u'key']))

        # Start a coroutine that runs a blocking command for 3seconds
        @asyncio.coroutine
        def run():
            yield From(protocol.brpop([u'key'], 3))
...
class RedisPoolTest(TestCase):
    ...
    def test_connection_in_use(self):
        ...
        def test():
            ...
            yield From(connection.delete([u'unknown-key']))
            f = asyncio.async(
                connection.blpop([u'unknown-key']), loop=self.loop)
    ...
    def test_parallel_requests(self):
        ...
        def test():
            ...
            yield From(connection.delete([u'my-list']))
            ...
            def sink():
                for i in range(0, 5):
                    reply = yield From(connection.blpop([u'my-list']))
            ...
            def source():
                for i in range(0, 5):
                    yield From(connection.rpush(u'my-list', [str(i)]))
                    yield From(asyncio.sleep(.5, loop=self.loop))
    ...
    def test_select_db(self):
        ...
        def test():
            ...
            yield From(c1.set(u'key', u'A'))
            yield From(c2.set(u'key', u'B'))

            r1 = yield From(c3.get(u'key'))
            r2 = yield From(c4.get(u'key'))

            self.assertEqual(r1, u'A')
            self.assertEqual(r2, u'B')
    ...
    def test_in_use_flag(self):
        ...
        def test():
            ...
            for i in range(0, 10):
                yield From(connection.delete([u'my-list-%i' % i]))

            @asyncio.coroutine
            def sink(i):
                the_list, result = yield From(
                    connection.blpop([u'my-list-%i' % i]))
            ...
            with self.assertRaises(NoAvailableConnectionsInPoolError) as e:
                yield From(connection.delete([u'my-list-one-more']))
                yield From(connection.blpop([u'my-list-one-more']))
            self.assertIn(
                'No available connections in the pool', e.exception.args[0])
    ...
    def test_lua_script_in_pool(self):
        ...
        def test():
            ...
            # Register script
            script = yield From(connection.register_script(u"return 100"))
            self.assertIsInstance(script, Script)
